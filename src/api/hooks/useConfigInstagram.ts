...
const useConfigInstagram = ({ appId, setAppId, setAppSecret }: useConfigInstagramType) => {
  ...
  const { mutate: addAppId } = useMutation({
    ...
    onSuccess: _res => {
      ...
      const authorizeEndpoint = `https://www.instagram.com/oauth/authorize?client_id=${appId}&redirect_uri=${redirectUri}&scope=instagram_business_basic&response_type=code`
